...
/**
 * habboquest Page Plugin Leaderboard Pixels
 */

if ($habboquestEnable['habboquestPluginLeaderboardCredits'] == true) {
    $getPixels = $db->prepare("SELECT username, pixels, look FROM users ORDER BY credits DESC LIMIT 8");
    $getPixels->execute();
    if ($getPixels->RowCount() > 0) {
        while ($Pixels = $getPixels->fetch()) {
            echo "
            <div class='habboquest-leaderboard-placeholder'>
             <img style='margin-left: 13.5px; margin-top: 15px; z-index: 2; position: relative;' src='".xss("http://www.habbo.nl/habbo-imaging/avatarimage?figure={$Pixels['look']}&size=b&direction=3&head_direction=3&")."'></img>
